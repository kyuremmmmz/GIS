<x-guest-layout>
    <form method="POST" action="{{ route('register') }}">
        ...
        <div>
            ...
            <x-text-input id="Adminname" class="block w-full mt-1" type="text" name="Adminname" :value="old('Adminname')" required autofocus autocomplete="Adminname" />
            <x-input-error :messages="$errors->get('Adminname')" class="mt-2" />
        </div>

         <!-- Admin ID -->
         <div>
            <x-input-label for="adminID" :value="__('adminID')" />
            <x-text-input id="adminID" class="block w-full mt-1" type="text" name="adminID" :value="old('adminID')" required autofocus autocomplete="adminID" />
